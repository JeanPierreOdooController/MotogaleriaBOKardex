		...
		return """
			...
			CREATE OR REPLACE VIEW public.vst_kardex_fisico1_lote AS 
			 SELECT stock_move.product_uom,
					...
					END AS price_unit,
					CASE
						WHEN uom_uom.id <> uomt.id THEN round(((sml.quantity::double precision / uom_uom.factor::double precision) * uomt.factor::double precision )::numeric, 6)
						ELSE sml.quantity
					END AS product_qty,
				sml.location_id,
				...
				(uomt.name ->> 'es_PE')::varchar AS unidad,
				spl.name as lote,
				spl.id as lote_id,
			...
			sm.product_uom,
					...
					END AS price_unit,
					CASE
						WHEN uom_uom.id <> uomt.id THEN round((sml.quantity::double precision * uomt.factor::double precision )::numeric, 6)
						ELSE sml.quantity
					END AS product_qty,
			 ...
			 SELECT stock_move.product_uom,
					CASE
						...
						(
							...
							WHEN l_o.usage in ('inventorys','productions') then stock_move.price_unit_it::double precision / coalesce(rcr_e.sale_type,-1)
			...
			sm.product_uom,
					CASE
						...
						(
							...
							WHEN sls.usage in ('inventorys','productions') then sm.price_unit_it::double precision / coalesce(rcr_e.sale_type,-1)
...
CREATE OR REPLACE VIEW public.vst_kardex_fisico1_lote AS 
 SELECT stock_move.product_uom,
		...
		END AS price_unit,
		CASE
			WHEN uom_line.id <> uomt.id THEN round((sml.quantity::double precision * uomt.factor::double precision / uom_line.factor::double precision )::numeric, 6)
			ELSE sml.quantity
		END AS product_qty,
	sml.location_id,
	sml.location_dest_id,
   ...
   FROM stock_move
	...
	left join stock_lot spl on spl.id = sml.lot_id
	 JOIN uom_uom ON stock_move.product_uom = uom_uom.id
	 JOIN uom_uom uom_line ON sml.product_uom_id = uom_line.id
	 JOIN stock_location l_o ON l_o.id = sml.location_id
	 JOIN stock_location l_d ON l_d.id = sml.location_dest_id
	 JOIN stock_picking ON stock_move.picking_id = stock_picking.id
	 LEFT JOIN account_move invoice ON invoice.id = stock_picking.invoice_id
...
CREATE OR REPLACE VIEW public.vst_kardex_fisico_gastos_vinculados AS 
 ...
 SELECT pt.uom_id AS product_uom,
	...
	gvdd.flete + gvdd.advalorem AS price_unit,
	0 AS product_qty,
...
CASE WHEN COALESCE(mpit.check_gastos_vinculados,false) then sm.kardex_date + interval '1 second' else
	...
	--'00'::character varying(4) AS guia,
 	tok.code::character varying(4) as guia,
	aaait.name::text AS analitic_id,
	...
	( pt.name::text)::character varying AS producto,
	pp.default_code AS cod_pro,
	(uomt.name  ->> 'es_PE')::varchar  AS unidad,
	gvd.company_id,
   ...
   FROM landed_cost_it gvd
	 ...
	 JOIN kardex_parameter mpit on mpit.company_id = sm.company_id
	 LEFT JOIN type_operation_kardex tok ON sp.type_operation_sunat_id = tok.id
	 left join account_analytic_account aaait on aaait.id = sm.analytic_account_id	 
  WHERE gvd.state::text = 'done'::text and pt.detailed_type = 'product' and coalesce(pt.check_unidad_especifica,false) = false
 ...
 SELECT pt.uom_id AS product_uom,
	NULL::integer AS move_dest_id,
	((gvdd.flete + gvdd.advalorem)/totalsm.total)*sml.quantity AS price_unit,
	0 AS product_qty,
	NULL::integer AS location_id,
	sl.id AS location_dest_id,
	sp.picking_type_id,
...
CASE WHEN COALESCE(mpit.check_gastos_vinculados,false) then sm.kardex_date + interval '1 second' else
   ...
   FROM landed_cost_it gvd
	 ...
	 join (
	 		select sum(sml_x.quantity) as total,
	 		sm_x.id
	 		from stock_move_line sml_x
	 		inner join stock_move sm_x on sm_x.id = sml_x.move_id
	 		group by  sm_x.id
	 		) totalsm on totalsm.id = sm.id
	 JOIN stock_picking sp ON sp.id = sm.picking_id
	 JOIN product_product pp ON pp.id = sm.product_id
	 JOIN product_template pt ON pt.id = pp.product_tmpl_id
	 JOIN stock_location sl ON sl.id = sp.location_dest_id
	 ...
	 LEFT JOIN type_operation_kardex tok ON sp.type_operation_sunat_id = tok.id
	 left join account_analytic_account aaait on aaait.id = sm.analytic_account_id	 
  WHERE gvd.state::text = 'done'::text and pt.detailed_type = 'product' and coalesce(pt.check_unidad_especifica,false) = true
...
CREATE OR REPLACE VIEW public.vst_kardex_fisico_gastos_vinculados_dolar AS 
 SELECT pt.uom_id AS product_uom,
	NULL::integer AS move_dest_id,
	gvdd.flete_usd + gvdd.advalorem_usd AS price_unit,
	0 AS product_qty,
	NULL::integer AS location_id,
	sl.id AS location_dest_id,
	sp.picking_type_id,
...
CASE WHEN COALESCE(mpit.check_gastos_vinculados,false) then sm.kardex_date + interval '1 second' else
 ...
 SELECT pt.uom_id AS product_uom,
	NULL::integer AS move_dest_id,
 	((gvdd.flete_usd + gvdd.advalorem_usd)/totalsm.total)*sml.quantity AS price_unit,
	0 AS product_qty,
	NULL::integer AS location_id,
	sl.id AS location_dest_id,
	sp.picking_type_id,
   ...
   FROM ( SELECT vst_stock_move.u_origen AS origen,
			vst_stock_move.u_destino AS destino,
				CASE
					...
					ELSE getserial(account_invoice.nro_comp)
				END AS serial,
				CASE
					WHEN vst_stock_move.location_id IS NULL THEN getnumber(vst_stock_move.name)
					ELSE
					CASE
						WHEN vst_stock_move.invoice_id <> 0 AND vst_stock_move.location_id IS NOT NULL THEN getnumber(account_invoice.nro_comp)::character varying(10)
		   ...
		   FROM ( SELECT vst_kardex_fisico.product_uom,
				   ...
				   join product_template pt on pt.id = pp.product_tmpl_id
				   where coalesce(pt.check_unidad_especifica,false)=false
				   group by vst_kardex_fisico.product_uom,
					...
					vst_kardex_fisico.company_id
				UNION ALL
   					SELECT vst_kardex_fisico.product_uom,
					vst_kardex_fisico.price_unit,
					(vst_kardex_fisico.product_qty) as product_qty,
					vst_kardex_fisico.location_id,
					vst_kardex_fisico.location_dest_id,
					vst_kardex_fisico.picking_type_id,
					vst_kardex_fisico.product_id,
					...
					vst_kardex_fisico.company_id,
					NULL::integer as aml_id,
					vst_kardex_fisico.lote_id as lot_id
				   FROM vst_kardex_fisico_lote() vst_kardex_fisico
				   join product_product pp on pp.id = vst_kardex_fisico.product_id
				   join product_template pt on pt.id = pp.product_tmpl_id
				   where coalesce(pt.check_unidad_especifica,false)=true
				UNION ALL
...
CREATE OR REPLACE VIEW vst_kardex_fisico_valorado AS 
 SELECT t.almacen, t.categoria, t.producto, t.fecha, t.periodo, t.ctanalitica, 
		...
		t.id_origen, t.id_destino,t.company_id,t.invoice_id, t.aml_id, t.lot_id
	 FROM ( SELECT vst_kardex_fis_1_1.almacen, vst_kardex_fis_1_1.categoria, 
		...
		vst_kardex_fis_1_1.invoice_id , vst_kardex_fis_1_1.aml_id, vst_kardex_fis_1_1.lot_id
