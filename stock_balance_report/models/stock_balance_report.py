...
class StockBalanceReport(models.Model):
	...
	def get_balance_view(self):
		...
		if kardex_save_obj:
			...
			#		inner join stock_lot  on stock_production_lot.id = stock_move_line.lot_id 
		...
		self.env.cr.execute("""
			...
			left join (
				select stock_move_line.location_id,sum(stock_move_line.quantity_product_uom) as reservado, stock_move_line.product_id  from stock_move_line where stock_move_line.state in ('partially_available','assigned') 
