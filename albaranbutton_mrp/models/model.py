...
class stock_balance_report_multipleorigin(models.TransientModel):
    ...
    lote = fields.Many2one("stock.lot",string="Lote")
...
class alberan_button(models.Model):
    ...
    def action_name(self):
        ...
        self.env.cr.execute(""" 
        select picking_id, 'picking', quantity,quantity from stock_move_line 
        ...
        union all
            select sm.raw_material_production_id, 'production',sml.quantity,sml.quantity from stock_move_line sml
...
class StockBalanceReportLote(models.Model):
    ...
    def action_name(self):
        ...
        self.env.cr.execute(""" 
        select sml.picking_id, 'picking', sml.quantity,sml.quantity from stock_move_line sml 
        left join stock_lot on stock_lot.id = sml.lot_id 
        where stock_lot.id """ + ((" = " + str(self.lote.id) ) if self.lote else " is null " ) 
