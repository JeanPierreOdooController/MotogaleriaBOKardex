...
class product_template(models.Model):
	...
	def unlink(self):
		if self.env.user.has_group("product_template_restriction.group_product_template_restriction"):
			...
			raise UserError("No Tiene Los Permisos de 'Permite Creacion de Productos'")
	...
	def write(self,vals):
		if "detailed_type" in vals or "tracking" in vals:
			for i in self:
				sml = self.env["stock.move.line"].sudo().search([
					("product_id.product_tmpl_id","=", i.id  )
				])
				if sml.sudo().ids:
					raise UserError("Imposible modificar el tipo de producto si ya tiene movimientos detallados creados.")
