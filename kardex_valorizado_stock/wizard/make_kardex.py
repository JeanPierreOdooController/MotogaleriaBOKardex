...
class stock_balance_report_lote_costeo(models.Model):
	...
	lote = fields.Many2one('stock.lot',string='Lote',store=True)
...
class make_kardex_valorado_stock(models.TransientModel):
	...
	def default_get(self, fields):
		...
		locat_ids = self.env['stock.location'].search([('usage','in',('internal','transit','procurement'))])
	...
	def do_csvtoexcel(self):
		...
		cell = WriteOnlyCell(ws, value="SALDOS VALORADOS" + ('SOLES' if self.moneda == 'pen' else 'DOLARES') )
		...
		ws.merged_cells.ranges.add(get_column_letter(1)+ "1:" + get_column_letter(14) + '1')
		ws.append([cell,"","","","","","","","","","","","","",""])
...
select 
			...
			left join stock_lot spt on spt.id = ksp.lote
	...
	left join purchase_order po on po.id = pol.order_id
		...
		return self.env['popup.it'].get_file('Saldo Valorado ' + ('Soles' if self.moneda == 'pen' else 'Dolares') +  str(ahora.day) + '/' + str(ahora.month) + '/' + str(ahora.year) + '.xlsx',output_datas)
...
select 
	...
	left join purchase_order po on po.id = pol.order_id
		...
		if kardex_save_obj:
			...
			pc.name as categoria, uu.name->>'es_PE' as unidad, ksp.producto as product_id, ksp.almacen as almacen_id, spt.name as lote, spt.id as lote_id
			from kardex_save_period ksp 
			inner join stock_location sl on sl.id = ksp.almacen
			inner join product_product pp on pp.id = ksp.producto
			inner join product_template pt on pt.id = pp.product_tmpl_id
			...
			#		inner join stock_lot  on stock_lot.id = stock_move_line.lot_id 
			#		where stock_lot.id = """ +str(alm.lote.id)+ """ and 
		...
		self.env.cr.execute("""
			...
			left join stock_lot spl on spl.id = vstf.lote_id
			left join (
				select stock_move_line.location_id,sum(stock_move_line.quantity_product_uom) as reservado, stock_move_line.product_id,stock_move_line.lot_id  from stock_move_line left join stock_lot  on stock_lot.id = stock_move_line.lot_id where stock_move_line.state in ('partially_available','assigned') 
		...
		for item in todos:
			...
			rango_vencimiento = ""
			if item[5]:
				loteobj = self.env['stock.lot'].browse(item[5])
