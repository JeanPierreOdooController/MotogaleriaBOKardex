...
class account_move(models.Model):
	...
	purchase_ebill = fields.Char('Origen - Nro. de Compra',compute="get_purchase_ebill", inverse="_set_purchase_ebill", store=True)
	...
	def _set_purchase_ebill(self):
		for i in self:
			alltext = ""
			if self.env.company.origen_nro_compra != False:
				for l in i.invoice_line_ids:
					for m in l.sale_line_ids:
						if m.id and m.order_id.id:
							m.order_id.client_order_ref = i.purchase_ebill
	...
	def write(self,vals):
		...
		for i in self:
			if i.state  == 'done' and 'state' in vals:
				...
				if  i.description_picking and  textd  and (" - Lotes: " + textd) in i.description_picking:
					pass
				else:
...
class account_move_line(models.Model):
	...
	costo_kardex = fields.Float('Costo Kardex',digits=(12,2),compute="get_costo_kardex")
	margen_kardex = fields.Float('Margen Kardex',digits=(12,2),compute="get_costo_kardex")

	def get_costo_kardex(self):
		for i in self:
			costo = 0
			for l in i.sale_line_ids:
				for nm in l.move_ids:
					if nm.picking_id.invoice_id.id == i.move_id.id and nm.product_id.id == i.product_id.id:
						costo = nm.price_unit_it
			i.costo_kardex = costo
			i.margen_kardex = i.price_subtotal - (costo*i.quantity)
				...
				for nm in l.move_ids:
					for m in nm.move_line_ids:
						if m.lot_id.id and nm.picking_id.invoice_id.id == i.move_id.id:
							if self.env.company.fecha_caducidad != False:
								textdxx = " *Caducidad: " + str(m.lot_id.expiration_date or '')[:10] 
								alltext += ( m.lot_id.name if alltext== "" else (", " + m.lot_id.name) ) + textdxx
							else:
