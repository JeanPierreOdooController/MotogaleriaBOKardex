...
class stock_move(models.Model):
	...
	def _action_confirm(self, merge=True, merge_into=False):
		...
		if mrp_flag:
			sepuede= False
			for i in self:
				if i.production_id.id or i.raw_material_production_id.id:
					sepuede=True
			t = super(stock_move,self)._action_confirm(sepuede,merge_into)
		else:			
	...
	def button_validate(selfs):
		...
		for self in selfs:
			if not self.kardex_date:
				self.with_context({'permitido':1}).write({'kardex_date': self.date_done})
				for i in self:
					i.move_ids_without_package.with_context({'permitido':1}).write({'kardex_date': self.date_done})
			else:
				for i in self:
					i.move_ids_without_package.with_context({'permitido':1}).write({'kardex_date': self.kardex_date})
			self.add_costos()
		return t
