...
class make_kardex_valorado_formato_sunat(models.TransientModel):
	...
	def report_txt(self):
		...
		for linea in self.env.cr.fetchall():
			...
			move = move_obj
			guia = move.picking_id.nro_guia_compra

			xtipo = linea[2] if linea[2] else ''
			xserie = linea[3] if linea[3] else ''
			xnumero = linea[4] if linea[4] else ''
			xtipon =  (str(move_obj.picking_id.type_operation_sunat_id.code) if move_obj.picking_id.type_operation_sunat_id.id else '' )
			if xtipo == '' and guia:
				xtipo = '09'
				xserie = guia.split('-')[0]
		...
		return self.env['popup.it'].get_file(nombre,base64.b64encode(txt.encode("utf-8")))
	...
	def reportpdf_sunat(self):
		...
		return self.env['popup.it'].get_file('KardexSunat.pdf',base64.b64encode(b''.join(f.readlines())))
	...
	def reporteador(self,obj_move):
		...
		for line in self.env.cr.fetchall():
			if line[21] != almacen_index or line[9] != product_index:
				...
				resta_horas = datetime.timedelta(hours=5)
				nueva_fecha_hora = now - resta_horas
				fecha_hora_formateada = nueva_fecha_hora.strftime("%d/%m/%Y %H:%M:%S")
				c.drawString(10,width-20,fecha_hora_formateada)
				...
				c.drawString( 270,pos_inicial-104,producto_obj.uom_id.code_sunat.code if producto_obj.uom_id.code_sunat.id else '' )#producto_obj.uom_id.einvoice_06.code or '')
				...
				c.drawString( 10 ,pos_inicial, "DOCUMENTO DE TRASLADO," )
			...
			if primero:
				...
				c.drawString( 10 ,pos_inicial, self.particionar_text(str(date_ini) ,36) )
				c.drawString( 10+36 ,pos_inicial, self.particionar_text(  '',30) )
				c.drawString( 10+66 ,pos_inicial,self.particionar_text(  '',30) )
				c.drawString( 10+96 ,pos_inicial,self.particionar_text(  '',35) )

				c.drawString( 10+156 ,pos_inicial,self.particionar_text(  '16',35) )
				c.drawString( 216 ,pos_inicial,self.particionar_text(  'Saldo Inicial',150) )
				...
				c.drawRightString( 366+50-2 ,pos_inicial, '{:,.2f}'.format(decimal.Decimal ("%0.2f" % cantidadx )))
				c.drawRightString( 416+50-2 ,pos_inicial, '{:,.2f}'.format(decimal.Decimal ("%0.2f" % ((montox / cantidadx) if cantidadx!= 0 else 0) )))
				c.drawRightString( 466+50-2 ,pos_inicial, '{:,.2f}'.format(decimal.Decimal ("%0.2f" % montox )))

				c.drawRightString( 516+50-2 ,pos_inicial, '{:,.2f}'.format(decimal.Decimal ("%0.2f" % ( 0) )))
				c.drawRightString( 566+50-2 ,pos_inicial, '{:,.2f}'.format(decimal.Decimal ("%0.2f" % ( 0) )))
				c.drawRightString( 616+50-2 ,pos_inicial, '{:,.2f}'.format(decimal.Decimal ("%0.2f" % ( 0) )))

				c.drawRightString( 666+50-2 ,pos_inicial, '{:,.2f}'.format(decimal.Decimal ("%0.2f" % cantidadx )))
				c.drawRightString( 716+50-2 ,pos_inicial, '{:,.2f}'.format(decimal.Decimal ("%0.2f" % ((montox / cantidadx) if cantidadx!= 0 else 0) )))
				c.drawRightString( 766+50-2 ,pos_inicial, '{:,.2f}'.format(decimal.Decimal ("%0.2f" % montox )))
			...
			xtipo = line[2] if line[2] else ''
			xserie = line[3] if line[3] else ''
			...
			c.drawString( 10+36 ,pos_inicial, self.particionar_text( xtipo,30) )
			c.drawString( 10+66 ,pos_inicial,self.particionar_text( xserie,30) )
			c.drawString( 10+96 ,pos_inicial,self.particionar_text( xnumero,35) )

			c.drawString( 10+156 ,pos_inicial,self.particionar_text( line[8] if line[8] else '',35) )
			c.drawString( 216 ,pos_inicial,self.particionar_text( line[7] if line[7] else self.env.company.name_get()[0][1] ,160) )
	...
	def report_xls(self):
		...
		for line in self.env.cr.fetchall():
			if line[21] != almacen_index or line[9] != product_index:
				...
				if product_index != False:
					...
					ws.append(["","","","","","TOTAL", '{:,.2f}'.format(decimal.Decimal ("%0.2f" % (total_1) )) ,"", '{:,.2f}'.format(decimal.Decimal ("%0.2f" % (total_2) )), '{:,.2f}'.format(decimal.Decimal ("%0.2f" % (total_3) )),"", '{:,.2f}'.format(decimal.Decimal ("%0.2f" % (total_4) ))])
				...
				ws.append([""])
				ws.append([""])
				cell = WriteOnlyCell(ws, value='FORMATO 13.1: "REGISTRO DE INVENTARIO PERMANENTE VALORIZADO - DETALLE DEL INVENTARIO VALORIZADO"')
				...
				cell_dat9 = WriteOnlyCell(ws, value="MÉTODO DE VALUACIÓN")
				...
				ws.append([cell_dat8,producto_obj.uom_id.code_sunat.code or ''])
				...
				linea = [border(ws,u"Fecha"),border(ws,u"Tipo"),border(ws,u"Serie"),border(ws,u"Numero"),border(ws,u"Tipo de operación"),border(ws,u"Cliente / Proveedor"),border(ws,u"Entrada Cantidad"),border(ws,u"Entrada C.U."),border(ws,u"Entrada Costo"),border(ws,u"Salida Cantidad"),border(ws,u"Salida C.U."),border(ws,u"Salida Costo"),border(ws,u"Saldo Final Cantidad"),border(ws,u"Saldo Final C.U."),border(ws,u"Saldo Final Costo")]
			...
			if primero:
				...
				cantidadx =((line[15] if line[15] else 0) - (line[11] if line[11] else 0) +   (line[13] if line[13] else 0) )
				montox = ((line[16] if line[16] else 0) - (line[12] if line[12] else 0) +   (line[14] if line[14] else 0) )
				ws.append([str(line[1]) if line[1] else '',str(line[2]) if line[2] else '',str(line[3]) if line[3] else '',str(line[4]) if line[4] else '','16','Saldo Inicial',	'{:,.2f}'.format(decimal.Decimal ("%0.2f" % (cantidadx) )),  '{:,.2f}'.format(decimal.Decimal ("%0.2f" % ((montox / cantidadx) if cantidadx!= 0 else 0) )),  '{:,.2f}'.format(decimal.Decimal ("%0.2f" % (montox) )),  '{:,.2f}'.format(decimal.Decimal ("%0.2f" % (0) )),  '{:,.2f}'.format(decimal.Decimal ("%0.2f" % (0     ) )),  '{:,.2f}'.format(decimal.Decimal ("%0.2f" % (0) )),  	'{:,.2f}'.format(decimal.Decimal ("%0.2f" % (cantidadx) )),  	'{:,.2f}'.format(decimal.Decimal ("%0.2f" % ((montox / cantidadx) if cantidadx!= 0 else 0) )),  '{:,.2f}'.format(decimal.Decimal ("%0.2f" % (montox) ))])
			...
			ws.append([str(line[1]) if line[1] else '',str(line[2]) if line[2] else '',str(line[3]) if line[3] else '',str(line[4]) if line[4] else '',str(line[8]) if line[8] else '',str(line[7]) if line[7] else '','{:,.2f}'.format(decimal.Decimal ("%0.2f" % (line[11] if line[11] else 0) )),  '{:,.2f}'.format(decimal.Decimal ("%0.2f" % ( (line[12] if line[12] else 0)/(line[11] if line[11] else 1)     ) )),  '{:,.2f}'.format(decimal.Decimal ("%0.2f" % (line[12] if line[12] else 0) )),  '{:,.2f}'.format(decimal.Decimal ("%0.2f" % (line[13] if line[13] else 0) )),  '{:,.2f}'.format(decimal.Decimal ("%0.2f" % ( (line[14] if line[14] else 0)/(line[13] if line[13] else 1)     ) )),  '{:,.2f}'.format(decimal.Decimal ("%0.2f" % (line[14] if line[14] else 0) )),  '{:,.2f}'.format(decimal.Decimal ("%0.2f" % (line[15] if line[15] else 0) )),  '{:,.2f}'.format(decimal.Decimal ("%0.2f" % ( (line[16] if line[16] else 0)/(line[15] if line[15] else 1)     ) )),  '{:,.2f}'.format(decimal.Decimal ("%0.2f" % (line[16] if line[16] else 0) ))])
		...
		return self.env['popup.it'].get_file('KardexSunat.xlsx',base64.b64encode(output.read()))
