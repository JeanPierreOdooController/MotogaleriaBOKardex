...
from vst_kardex_fisico_valorado as vst_kardex_sunat
	   ...
	   where (fecha_num((vst_kardex_sunat.fecha- interval '5' hour)::date) between """+str(date_ini).replace('-','')+""" and """+str(self.fecha_final).replace('-','')+""")    
		...
		for xl in total_all:
			...
			if l['stock_moveid'] and l['origen']:
				if mrp_flag:
					self.env.cr.execute("""
						...
						where sm.id = """ +str(l['stock_moveid'])+ """
      						""" +  ((""" and sml.lot_id = """ +str(l['lot_id']) ) if l['lot_id'] else ''  )+"""
					 """)
			...
			if l['stock_moveid'] and str(l['fecha'])[0:10]>= str(self.fecha_inicio)[0:10]:
				...
				if (data_temp['origen'] == 'internal' and data_temp['destino'] == 'transit') or (data_temp['origen'] == 'internal' and data_temp['destino'] == 'production') or (data_temp['origen'] == 'internal' and data_temp['destino'] == 'customer'):
					...
					self.env.cr.execute(""" UPDATE stock_move set price_unit_it = """ +str( cprom_act_antes )+ """  where stock_move.picking_id in (select id from stock_picking where origin like '%""" +str( data_temp['name'] )+ """%' or origin = 'Retorno de """ +str( data_temp['name'] )+ """' ) and product_id = """ +str(l['product_id'])+ """; """)
					if data_temp['name2']:						
			...
			if l['stock_moveid'] and l['origen']:
				if mrp_flag:
					self.env.cr.execute(""" 
						...
						select sm.production_id,sm.id,sl_o.usage as origen , sl_d.usage as destino, sp.name,sm.origin, coalesce(sm.price_unit_it_dolar,0)  as price_unit_it, sp2.name as name2, sp3.name as name3 from
						stock_move sm
						inner join stock_move_line sml on sml.move_id =  sm.id
						left join stock_picking sp on sp.id = sm.picking_id
						left join stock_picking sp2 on sp2.id = sp.backorder_id
