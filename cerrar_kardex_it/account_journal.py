...
class stock_picking(models.Model):
	...
	def create(self,vals):		
		...
		if len(vals.keys() )== 1:
			...
			t.verifycrd('CreateSP',str(vals))
		return t

	def verifycrd(self,tipo,vals):
		for t in self:
			if t.kardex_date  and t.state =='done':
				for i in self.env['kardex.cerrado.config'].search([('company_id','=',self.env.company.id)]):
					if str(t.kardex_date - timedelta(hours=5) if t.kardex_date else t.kardex_date)[:10] >= str(i.fecha_inicio) and str(t.kardex_date - timedelta(hours=5) if t.kardex_date else t.kardex_date)[:10] <= str(i.fecha_fin):
						raise UserError("Alerta! El kardex fue cerrado para este fecha y Ubicación. Origen:" + tipo +' ,Valores: '+ vals)
	...
	def write(self,vals):
		...
		if len(vals.keys() )== 1:
			if 'invoice_id' in vals:
				return m
		for i in self:
			i.verifycrd('WriteSP',str(vals))
		return m

	
	def unlink(self):
		for i in self:
			i.verifycrd('UnlinkSP','{}')
