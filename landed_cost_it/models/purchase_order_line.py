...
class PurchaseOrderLine(models.Model):
	...
	tc_landed = fields.Float(string='TC',compute='compute_tc_landed',digits=(12,4),store=False)
	price_total_signed_landed = fields.Float(string='Total Soles',compute='compute_price_total_signed_landed',digits=(64,2),store=False)
	...
	@api.depends('order_id.date_order','order_id.company_id')
	def compute_tc_landed(self):
		for line in self:
			currency = self.env.ref('base.USD')
			tc = self.env['res.currency.rate'].search([('name','=',str(line.order_id.date_order.date())),('currency_id','=',currency.id),('company_id','=',line.order_id.company_id.id)],limit=1)
			line.tc_landed = tc.sale_type if tc else 1
	
	@api.depends('order_id.currency_id','price_subtotal','tc_landed')
