...
class make_kardex_valorado(models.TransientModel):
	...
	def do_csvtoexcel(self):
		...
		ws.column_dimensions['H'].width = 40.29
		...
		cell = WriteOnlyCell(ws, value="KARDEX VALORADO " + ('SOLES' if self.moneda == 'pen' else 'DOLARES') )
		...
		ws.merged_cells.ranges.add(get_column_letter(5)+ "1:" + get_column_letter(10) + '1')
		ws.append(["","","","",cell])
		...
		ws.merged_cells.ranges.add(get_column_letter(13)+ "8:" + get_column_letter(14) + '8')

		ws.merged_cells.ranges.add(get_column_letter(15)+ "8:" + get_column_letter(16) + '8')

		ws.merged_cells.ranges.add(get_column_letter(17)+ "8:" + get_column_letter(18) + '8')
...
select 
				...
				0 as credit,
				(ksp.fecha || ' 00:00:00')::timestamp as fechax,
	...
	left join (
		...
		for xl in total_all:
			...
			llave = (l['product_id'],l['location_id'],l['lot_id'])
			cprom_acum = [0,0]
		...
		return self.env['popup.it'].get_file('Kardex Valorado ' + ('Soles' if self.moneda == 'pen' else 'Dolares') + ' ' + str(ahora.day) + '/' + str(ahora.month) + '/' + str(ahora.year) + '.xlsx',output_datas)
