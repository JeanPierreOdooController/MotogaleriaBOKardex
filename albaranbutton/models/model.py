...
class StockBalanceReportLote(models.Model):
    ...
    def action_name(self):
        ...
        self.env.cr.execute(""" select distinct picking_id from stock_move_line left join stock_lot on stock_lot.id = stock_move_line.lot_id where stock_lot.id """ + ((" = " + str(self.lote.id) ) if self.lote else " is null " ) +  """ and stock_move_line.location_id = """ +str(self.almacen_id.id)+ """ and stock_move_line.product_id = """ +str(self.product_id.id)+ """ and stock_move_line.state in ('partially_available','assigned') """)
    ...
    def action_so_name(self):
        ...
        self.env.cr.execute(""" select distinct sale_id from stock_move_line sml  left join stock_lot on stock_lot.id = sml.lot_id inner join stock_picking sp on sp.id = sml.picking_id where stock_lot.id  """ + ((" = " + str(self.lote.id) ) if self.lote else " is null " ) +  """ and sml.location_id = """ +str(self.almacen_id.id)+ """ and sml.product_id = """ +str(self.product_id.id)+ """ and sml.state in ('partially_available','assigned') """)
