...
class stock_picking(models.Model):
	...
	tc_personalizado = fields.Boolean('Tipo de cambio personalizado',default=False)
	...
	def write(selfs,vals):
		...
		for self in selfs:
			if 'nomore' in self.env.context:
				...
				if self.tc_personalizado== False:
					fecha_pedido = False
					moneda = False
					compra=False
					import datetime
	...
	#				if (self.state == 'done' and fecha_pedido and moneda == 'USD' and self.tc == 1) or (self.state == 'done' and fecha_pedido and moneda == 'USD' and 'invoice_id' in vals):
					if (moneda!=False and self.state == 'done' and fecha_pedido and moneda != 'PEN') or (moneda!=False and self.state == 'done' and fecha_pedido and moneda != 'PEN' and 'invoice_id' in vals):
						if self.location_id.usage == 'internal':
							pass
						else:
							tp = self.env['res.currency.rate'].search([('currency_id.name','=',moneda),('name','=', fecha_pedido )])
							if len(tp)==0 and not self.invoice_id.id:
								raise UserError('No existen el tipo de cambio para ' + fecha_pedido.strftime("%Y-%m-%d") + ' del pedido de compra en ' +moneda+ ' del albaran ' + self.name)						
							if len(tp)==0 and self.invoice_id.id:
								raise UserError('No existen el tipo de cambio para ' + self.invoice_id.invoice_date.strftime("%Y-%m-%d") + ' de la facura en ' +moneda+ ' del albaran ' + self.name)						
...
#				if (self.state == 'done' and fecha_pedido and moneda == 'USD' and self.tc == 1) or (self.state == 'done' and fecha_pedido and moneda == 'USD' and 'invoice_id' in vals):
			if (self.state == 'done' and fecha_pedido and moneda != 'PEN'):
				if self.location_id.usage == 'internal':
					pass
				else:
					tp = self.env['res.currency.rate'].search([('currency_id.name','=',moneda),('name','=', fecha_pedido )])
	...
	def write(selfs,vals):
		...
		for self in selfs:
			if 'nomore' in self.env.context:
				...
				if 'price_unit_it' in vals and self.picking_id.id:
					self.env.cr.execute("""
						update stock_move set price_unit_it = """ +str(vals['price_unit_it'])+ """ where picking_id in (select id from stock_picking where return_id = """+str(self.picking_id.id)+""" ) and 
